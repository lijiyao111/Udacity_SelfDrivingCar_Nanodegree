 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for( int i = 0; i < int(N); i++ ) {
      fg[0] += 20000*CppAD::pow(vars[cte_start + i], 2);
      fg[0] += 20000*CppAD::pow(vars[epsi_start + i], 2);
    ...
    for (int i = 0; i < int(N) - 2; i++) {
      fg[0] += 80000000*CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
    ...
    for (int t = 1; t < int(N); t++) {
      ...
      // reflect the vehicle state transition equations
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for ( int i = delta_start; i < int(a_start); i++ ) {
    vars_lowerbound[i] = -0.436332; // 25 degree in radians
